...
def test_parse_weekly_review_day() -> None:
    assert flow._parse_weekly_review_day("SUN") == 6
    assert flow._parse_weekly_review_day("monday") == 0
    assert flow._parse_weekly_review_day(3) == 3
    assert flow._parse_weekly_review_day("nope") is None
    assert flow._parse_weekly_review_day(8) is None


def test_next_weekly_run_future_weekday() -> None:
    now = datetime(2026, 1, 22, 9, 0, tzinfo=timezone.utc)  # Thursday
    target = flow._next_weekly_run(now, 6, time(10, 0))  # Sunday
...
def test_next_midnight_run_rolls_forward() -> None:
    ...
    target = flow._next_midnight_run(now)
    assert target == datetime(2026, 1, 23, 0, 0, tzinfo=timezone.utc)


def test_parse_due_time_reminder_offsets() -> None:
    assert flow._parse_due_time_reminder_offsets([120, 15, 120, "15", "abc", -1, 0]) == (120, 15)
    assert flow._parse_due_time_reminder_offsets(None) == ()
    assert flow._parse_due_time_reminder_offsets("120") == ()


def test_parse_due_time_reminder_offsets_ignores_booleans() -> None:
    assert flow._parse_due_time_reminder_offsets([True, False, 15]) == (15,)


def test_load_due_time_reminder_schedule_config_defaults() -> None:
    schedule = flow._load_due_time_reminder_schedule_config({})
...
def test_load_due_time_reminder_schedule_config_explicit_empty_disables() -> None:
    schedule = flow._load_due_time_reminder_schedule_config({"due_time_reminder_offsets_minutes": []})
...
def test_notify_due_time_reminder_schedule_changed_calls_runtime_callback() -> None:
    ...
    config = {flow._DUE_TIME_REMINDER_NOTIFY_CONFIG_KEY: _callback}
    flow._notify_due_time_reminder_schedule_changed(config)
    flow._notify_due_time_reminder_schedule_changed(config, clear_state=True)
...
def test_due_time_reminder_dispatch_requeues_on_send_failure(monkeypatch) -> None:
    async def _run() -> None:
        ...
        bot = flow.SquireBot(config=config)
        now = datetime.now(timezone.utc)
        event = flow.DueTimeReminderEvent(
            object_id="A_1",
            title="Call vet",
        ...
        async def _fake_resolve_channel():
            class _Channel:
                async def send(self, content):
                    raise flow.discord.HTTPException(SimpleNamespace(status=500, reason="failed"), "failed")
        ...
        monkeypatch.setattr(flow, "render_due_time_reminder_message", lambda *args, **kwargs: "msg")
...
def test_due_time_reminder_loop_flushes_empty_ledger_on_clear_state(monkeypatch) -> None:
    async def _run() -> None:
        ...
        bot = flow.SquireBot(config=config)
        bot._due_time_reminder_sent_ledger["old"] = flow._DueTimeReminderSentLedgerEntry(
            key="old",
            object_id="A_1",
...
def test_due_time_reminder_dispatch_skips_duplicate_from_sent_ledger(monkeypatch) -> None:
    async def _run() -> None:
        ...
        duplicate_key = flow._due_time_reminder_key(event)
        bot._due_time_reminder_sent_ledger[duplicate_key] = flow._DueTimeReminderSentLedgerEntry(
            key=duplicate_key,
...
def test_handle_command_weekly(monkeypatch) -> None:
    ...
    handled = asyncio.run(flow._handle_command(object(), "!weekly", "R_1", config))
