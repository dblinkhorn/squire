...
def test_parse_health_port_defaults_and_disable() -> None:
    assert discord_bot._parse_health_port(None) == 8080
    assert discord_bot._parse_health_port("") == 8080
    assert discord_bot._parse_health_port("9090") == 9090
    assert discord_bot._parse_health_port("0") is None


def test_parse_health_port_rejects_invalid_values() -> None:
    with pytest.raises(ValueError):
        discord_bot._parse_health_port("abc")

    with pytest.raises(ValueError):
        discord_bot._parse_health_port("70000")


def test_health_endpoint_reports_ok() -> None:
    server = discord_bot._HealthServer("127.0.0.1", 0)
    server.start()
    try:
